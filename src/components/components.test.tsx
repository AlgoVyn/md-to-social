...
describe("StyleModal", () => {
  ...
  it("should call setFormatStyle when style option is selected", async () => {
    ...
    expect(defaultProps.setFormatStyle).toHaveBeenCalledWith(
      "bullet-optimized",
    );
  });
  ...
  it("should highlight the currently selected style", () => {
    const { rerender } = render(
      <StyleModal {...defaultProps} formatStyle="bullet-optimized" />,
    );

    // The bullet-optimized option should have active class
    const bulletOption = screen
      .getByDisplayValue("bullet-optimized")
      .closest("label");
    expect(bulletOption).toHaveClass("active");

    rerender(<StyleModal {...defaultProps} formatStyle="bold-headers" />);

    const boldHeadersOption = screen
      .getByDisplayValue("bold-headers")
      .closest("label");
...
describe("HistoryModal", () => {
  ...
  it("should display drafts with preview and timestamp", () => {
    ...
    expect(
      screen.getByText(/Draft two content that is longer/),
    ).toBeInTheDocument();
  });
  ...
  it("should truncate long draft content", () => {
    const longDrafts: Draft[] = [
      { id: "1", markdown: "a".repeat(150), updatedAt: Date.now() },
    ];
...
describe("LinkedInPost", () => {
  it("should render post structure", () => {
    ...
    expect(
      screen.getByText("Content Creator at SocialBoost"),
    ).toBeInTheDocument();
